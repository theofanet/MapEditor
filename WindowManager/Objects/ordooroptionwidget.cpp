#include "ordooroptionwidget.h"
#include "ui_ordooroptionwidget.h"

OrDoorOptionWidget::OrDoorOptionWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::OrDoorOptionWidget)
{
    setFixedWidth(300);
    ui->setupUi(this);
...
OrDoorOptionWidget::~OrDoorOptionWidget()
{
    delete ui;
}


void OrDoorOptionWidget::setMap(Map *m){
    if(!_map || _map != m){
        _map = m;
        if(_map)
            connect(_map, SIGNAL(orDoorTargetSelected(Object *)), this, SLOT(targetSelected(Object*)));
    }
}

void OrDoorOptionWidget::selectTargetClicked(bool){
    if(_map)
        _map->setEditionMode(EDIT_MODE_OR_DOOR_SELECT_TARGET);
}

void OrDoorOptionWidget::targetSelected(Object *target){
...
void OrDoorOptionWidget::connectDoor(Object *target, string cmd){
...
void OrDoorOptionWidget::setOrDoor(OrDoor *d){
    _door = d;
    if(_door){
